    ...
    <div className="group relative cursor-pointer overflow-hidden rounded-xl">
      ...
      <div className="relative z-30 px-3 pb-3 pt-[7rem] text-white">
        ...
        <Link href={`/article/${item.slug}`}>
          <h2 className="py-1 text-2xl font-bold leading-[1.25] transition-colors hover:text-accent">
            {truncateString(item?.title, 50) + "..."}
          </h2>
        </Link>

        <p className="pb-4 font-medium text-white/90 shadow-sm">
        ...
        <Link href={`/article/${item.slug}`}>
          <p className="pb-[3.25rem] text-[.9rem] font-extralight leading-[1.65] text-white/90 drop-shadow-sm">
            ...
            item.category.description?.length > 150 ? (
              <>
                {truncateString(item.category.description, 150)}
